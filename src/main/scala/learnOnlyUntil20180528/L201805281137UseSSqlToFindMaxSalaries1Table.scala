...
object L201805281137UseSSqlToFindMaxSalaries1Table {

  def main(args: Array[String]) {

    Logger.getLogger("org").setLevel(Level.ERROR)
    val session = SparkSession.builder().appName("L201805281252ShufflingUseSSqlToFindMaxSalaries").master("local[1]").getOrCreate()
    ...
    //val simpleJoin = session.sql("SELECT s.yearID, s.salary, COUNT(s.salary) Count_Salary FROM salaries s GROUP BY s.yearID, s.salary ORDER BY s.yearID, s.salary")
